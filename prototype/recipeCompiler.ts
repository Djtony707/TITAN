 ...
 * tool plan with typed parameter slots that can be replayed with ZERO model
 * calls. The local smollm2-360m does slot-filling only — that's Fizz's
 * "100th invocation goes to ~zero tokens" claim, made concrete.
 *
 ...
 *
 * The slot-filling model is NOT wired here (no smollm2 in this sandbox);
 * the prototype marks slots as `{{paramName}}` in the v7 recipe interpolation
 * syntax. In the real build, a single 360m call replaces the `{{param}}`
 * tokens with values extracted from the user's one-word intent.
 */
