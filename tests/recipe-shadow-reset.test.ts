...
describe('promotion state machine hardening', () => {
    it('resets shadow counters and epoch on re-entry to shadow after demotion', () => {
        ...
        for (let i = 0; i < SHADOW_MIN_COMPARISONS; i++) {
            recordShadowComparison(recipe.id, { epoch: getEntry(recipe.id)!.recipe.stats.shadowEpoch, comparisonId: `xval-${++__xv}`, recipe: [{ name: 'out', content: 'ok' }], frontier: [{ name: 'out', content: 'ok' }] });
        }
        activate(recipe.id);
    ...
    it('verdict is computed from outputs, not caller-supplied boolean', () => {
        ...
        const result = recordShadowComparison(recipe.id, { epoch: getEntry(recipe.id)!.recipe.stats.shadowEpoch, comparisonId: `xval-${++__xv}`, recipe: [{ name: 'out', content: 'different' }], frontier: [{ name: 'out', content: 'same' }] });
        expect(result.equivalent).toBe(false);
        expect(getEntry(recipe.id)!.recipe.stats.shadowSuccesses).toBe(0);

        const result2 = recordShadowComparison(recipe.id, { epoch: getEntry(recipe.id)!.recipe.stats.shadowEpoch, comparisonId: `xval-${++__xv}`, recipe: [{ name: 'out', content: 'same' }], frontier: [{ name: 'out', content: 'same' }] });
