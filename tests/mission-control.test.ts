...
vi.mock("../src/config/config.js", () => ({
  loadConfig: vi.fn().mockImplementation(() => ({
    ...
    channels: {
      ...
      webchat: { enabled: true, dmPolicy: "pairing" },
    },
    gateway: {
      port: TEST_PORT,
      host: "127.0.0.1",
...
vi.mock("../src/memory/memory.js", () => ({
  ...
  getUsageStats: vi
    .fn()
    .mockReturnValue({ sessions: 5, messages: 100, tokens: 50000 }),
...
vi.mock("../src/utils/updater.js", () => ({
  getUpdateInfo: vi
    .fn()
    .mockResolvedValue({ current: "6.1.0-alpha.22", latest: "6.1.0-alpha.22", upToDate: true }),
}));
...
describe("Mission Control v2", () => {
  ...
  describe("React SPA Serving", () => {
    it('GET / → HTML with <div id="root"> (React mount point)', async () => {
      ...
      const uiIndexPath = new URL("../ui/dist/index.html", import.meta.url)
        .pathname;
  ...
  describe("Health & System", () => {
    it('GET /api/health → { status: "ok", version: "2026.10.0" }', async () => {
      ...
      expect(body.version).toBe("6.1.0-alpha.22");
  ...
  describe("Overview Panel", () => {
    ...
    it("GET /api/update → current version info", async () => {
      ...
      expect(res.status).toBe(200);
      const body = (await res.json()) as any;
      expect(body.current).toBe("6.1.0-alpha.22");
    });
  });
  ...
  describe("Skills Panel", () => {
    ...
    it("POST /api/skills/:name/toggle → toggles skill", async () => {
      const res = await fetch(`${BASE}/api/skills/shell/toggle`, {
        method: "POST",
      });
      expect(res.status).toBe(200);
    });
  ...
  describe("Sessions Panel", () => {
    ...
    it("POST /api/sessions/:id/close → closes session", async () => {
      const res = await fetch(`${BASE}/api/sessions/sess-1/close`, {
        method: "POST",
      });
      expect(res.status).toBe(200);
    });
