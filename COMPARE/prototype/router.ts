 ...
 * zero frontier calls for eligible semantically verified replay.
 *
 * Safety invariant: high-stakes recipes (destructive tools: shell, exec, etc.)
 * NEVER auto-replay, even on exact match with perfect slot fill. They return
 * a 'confirm' decision and the caller must prompt the user. This is enforced
 * inside route() — not just metadata on the recipe.
 *
...
export interface RouterInput {
     ...
     * real build this comes from a single local model call that extracts
...
export interface RouterDecisionConfirm {
    kind: 'confirm';
    recipe: CompiledRecipe;
    resolvedArgs: Array<{ tool: string; args: Record<string, unknown> }>;
    stakes: StakesLevel;
...
export type RouterDecision = RouterDecisionReplay | RouterDecisionMiss | RouterDecisionConfirm;
 ...
 * false positives on the zero-frontier-call path.
 *
...
export function route(input: RouterInput): RouterDecision {
    ...
    for (const recipe of input.activeRecipes) {
        ...
        // The user must approve; the recipe pre-fills the plan.
        const stakes = stakesFor(recipe, input.stakesOverride);
        if (stakes === 'high') {
            // Even with slot fill, high-stakes recipes return a confirm
            // decision, not a replay. The caller (agentLoop) must prompt
            // the user. This is the router's safety invariant: no
            // destructive tool ever fires without explicit human OK.
            ...
            return {
                ...
                stakes,
                message: `Recipe "${recipe.name}" matches but includes high-stakes tools. User confirmation required before replay.`,
            };
        }
        ...
        // 4. Replay! Zero frontier calls for this turn (low/medium stakes only).
        const tokensSaved = 0;
        return {
            kind: 'replay',
            recipe,
