...
- **Real reminders.** "Remind me Friday at 5pm to buy charcoal" now actually works: a `reminder` tool (set/list/cancel; relative asks pass `inMinutes` so models never do timezone math) + a 60-second watcher that delivers due reminders through the heartbeat channel — the mascot announces them. Proven live end-to-end.
- **An agent that can't lie about it.** Honesty steering plus a deterministic anti-fabrication guard: if a reply claims a reminder/schedule was set and no scheduling-capable tool ran that turn, TITAN appends a visible correction. Found via act-like-a-user testing, where the model fabricated "I've scheduled a reminder" — structurally impossible to slip through now.
...
- **Goal driver: burst dispatch.** `src/agent/goalDriver.ts` now detects when 2+ subtasks are ready (deps satisfied, no shared data) and fires them via `Promise.all` instead of one-per-tick. Up to 4 subtasks run concurrently. Mission Chat goals with independent sub-goals now complete in roughly `max(subtask_durations)` instead of `sum(subtask_durations)`. Adds `pickAllReadySubtasks(goal, state, cap)` for the burst path; the legacy `pickNextReadySubtask` is now a thin wrapper for the single-subtask path.
- **Goal driver: safety design.** Burst mode only handles the happy path. Any non-success outcome (failed / needs_info / blocked / thrown) leaves the subtask in `pending` state so the next tick re-enters the single-subtask flow, where all the existing edge-case guards (commit-override, repeat-question dedupe, generic-question pre-pass, consecutive-needs-info counter, assertion-failure tracking) still apply. Burst is a fast-path overlay, not a replacement.
- **Goal driver: state shape.** Adds `state.parallelBatch?: { ids: string[]; startedAt: string }` so observers can distinguish single in-flight from burst in-flight without re-deriving from `subState.pendingSpawn`. Emits `burst: true` + `burstSize` on `agent_spawn` / `agent_done` events for the Mission Chat lifecycle bridge.
- **Planner: non-linear deps.** `decomposeGoalIntoTasks` (extracted from `plan_task`) now classifies the connector between steps before setting `dependsOn`. Coordinating connectors (`and`, `,`, `;`) produce empty `dependsOn` (independent → parallel-ready); ordering connectors (`then`, `after`, `finally`) produce `[previousId]`. Pre-v6.3 every step had `dependsOn: [previousId]` regardless of intent, so the new burst path would have been dead code without this fix.
- **Orchestrator: regex gate removed.** `src/agent/orchestrator.ts` previously required user messages to match one of five hard-coded regex patterns ("research X and write Y", "multiple/parallel/simultaneously", etc.) before invoking the LLM delegation classifier. Real multi-step missions like "build a dashboard with charts and a backend" matched none of them. The regex allowlist is gone; the cheap pre-filter now only catches greetings, lookup-style questions, and status acks (`isObviouslyTrivial`). Substantive ≥10-word messages go to the LLM classifier and let it judge intent.
- **System prompt: stronger `agent_team` directive.** `DELEGATION_BLOCK` rewritten to make `agent_team` the default behavior for any 2+ sub-goal request, with concrete trigger examples (comma lists, "X and Y", parallel compares, cross-file audits). Trimmed to stay under the 8KB system-prompt budget.
- **Tests.** 24 new tests across 3 files: `tests/planner-decompose.test.ts` (10 — connector classification, ordering vs coordinating, single-step fallback), `tests/orchestrator-gate.test.ts` (6 — cheap pre-filter, LLM classifier dispatch on previously-missed shapes), `tests/agent/pickAllReadySubtasks.test.ts` (8 — cap respect, dep satisfaction, declaration order).
