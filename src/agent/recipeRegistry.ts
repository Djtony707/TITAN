...
export function promoteToShadow(id: string, reason = 'entered shadow evaluation'): RegistryEntry {
    ...
    // Start a fresh shadow window. Old comparison evidence is discarded so a
    // previously-demoted recipe cannot reactivate on stale data.
    entry.recipe.stats.shadowComparisons = 0;
    entry.recipe.stats.shadowSuccesses = 0;
    entry.recipe.stats.shadowEpoch = (entry.recipe.stats.shadowEpoch || 0) + 1;
...
export interface ShadowRunResult {
    /** Canonical recipe output from the shadow replay. */
    recipeOutput: unknown;
    /** Frontier/model output we compare against. */
    frontierOutput: unknown;
}

function defaultEquivalent(a: unknown, b: unknown): boolean {
    if (a === b) return true;
    if (typeof a !== typeof b) return false;
    if (a == null || b == null) return a === b;
    if (Array.isArray(a) && Array.isArray(b)) {
        if (a.length !== b.length) return false;
        return a.every((x, i) => defaultEquivalent(x, (b as unknown[])[i]));
    }
    if (typeof a === 'object' && typeof b === 'object') {
        const ka = Object.keys(a as object);
        const kb = Object.keys(b as object);
        if (ka.length !== kb.length) return false;
        return ka.every(k => defaultEquivalent((a as Record<string, unknown>)[k], (b as Record<string, unknown>)[k]));
    }
    return false;
}
...
export function recordShadowComparison(
    ...
    options?: { equivalent?: (a: unknown, b: unknown) => boolean },
): { epoch: number; equivalent: boolean; comparisons: number; successes: number } {
    const reg = load();
    const entry = reg.entries[id];
    if (!entry) throw new Error(`RecipeRegistry: unknown recipe ${id}`);
    if (entry.recipe.state !== 'shadow') {
    ...
    const equivalent = (options?.equivalent ?? defaultEquivalent)(runs.recipeOutput, runs.frontierOutput);
    const epoch = entry.recipe.stats.shadowEpoch;
    entry.recipe.stats.shadowComparisons++;
    if (equivalent) entry.recipe.stats.shadowSuccesses++;
    save(reg);
    return { epoch, equivalent, comparisons: entry.recipe.stats.shadowComparisons, successes: entry.recipe.stats.shadowSuccesses };
}
