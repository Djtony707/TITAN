...
const HTML_REPORT_GUIDANCE = [
    ...
    '    • Images — MANDATORY workflow (alpha.53):',
    ...
    '        2. Call `download_image({ url: "<the URL>" })`. It returns `{ ok: true, dataUrl: "data:image/jpeg;base64,…", sizeBytes, mimeType }`. The dataUrl is portable — it embeds the bytes directly in the HTML, no network request needed.',
    '        3. Use that dataUrl AS-IS as the src: `<img src="data:image/jpeg;base64,…" alt="…" style="max-width:520px;display:block;margin:18px auto;border-radius:6px;">`.',
    '      ONLY two acceptable `<img>` forms in your final HTML — (a) a `data:` URL returned by download_image, or (b) an inline hand-authored `<svg>…</svg>`. NEVER emit `<img src="https://…">` or `<img src="http://…">` — those will be REPLACED with a "image unavailable" placeholder block at the viewer (server enforces this in code, not by asking nicely). If download_image returns `{ ok: false, error: … }`, try a different source URL (Wikimedia/NASA/government domains are most reliable) OR omit the image and lean on the prose — never fabricate a URL.',
