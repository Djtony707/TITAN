...
const HTML_REPORT_GUIDANCE = [
    ...
    '    • Images: ONLY two acceptable forms — (a) `<img src="data:image/…;base64,…" alt="…">` after calling **download_image({ url })** on a real source URL, or (b) inline `<svg>…</svg>` you hand-author. NEVER emit `<img src="https://…">` or `<img src="http://…">` — the viewer enforces this and replaces any raw external `<img>` with a placeholder block (your image will NOT render). If you can\'t download a real image, hand-build an SVG illustration OR omit the image and lean on the prose. Fabricated image URLs guarantee a broken placeholder.',
